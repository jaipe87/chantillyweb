...
class TortasController
{
    ...
    public function refresh_datos_tortas_destacadas()
    {
        ...
        try {
            ...
            $tbltortas = $daoProductoweb->select_all_Productos_destacados(["cia" => $this->cia, "pagina"=> $id_pagina,"criterio"=>$criterio], $offset, $limite);
            $total_items = $daoProductoweb->select_count_productos_destacados(["cia" => $this->cia, "pagina"=> $id_pagina,"criterio"=>$criterio]);
            $data = [];
            $id = [];
    ...
    public function refresh_datos_tortas_personalizadas()
    {
        header('Content-Type: application/json');
        try {
            $daoProductoweb = new daoProducto_web($this->DB);
            ...
            // Lógica de paginación
            $offset = ($pagina - 1) * $limite;
            $tbltortas = $daoProductoweb->select_all_Productos_personalizadas(["cia" => $this->cia, "pagina"=> $id_pagina], $offset, $limite);
            $total_items = $daoProductoweb->select_count_productos_personalizadas(["cia" => $this->cia, "pagina"=> $id_pagina]);
            $data = [];
            $id = [];
            if (is_array($tbltortas) && count($tbltortas) > 0) {
                foreach ($tbltortas as $fila) {
    ...
    public function refresh_datos_tortas()
    {
        header('Content-Type: application/json');
        try {
            $daoProductoweb = new daoProducto_web($this->DB);
            ...
            // Lógica de paginación
            $offset = ($pagina - 1) * $limite;
            $tbltortas = $daoProductoweb->select_all_Productos_tortas(["cia" => $this->cia, "pagina"=> $id_pagina,"criterio"=>$criterio], $offset, $limite);
            $total_items = $daoProductoweb->select_count_productos_tortas(["cia" => $this->cia, "pagina"=> $id_pagina,"criterio"=>$criterio]);
            $data = [];
            $id = [];
            if (is_array($tbltortas) && count($tbltortas) > 0) {
                foreach ($tbltortas as $fila) {

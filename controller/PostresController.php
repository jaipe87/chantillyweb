...
class PostresController
{
    ...
    public function Index()
    {
        try {
            ...
            $titulo = "POSTRES EN LÍNEA";
            $this->tpl->assign("titulo", $titulo);
            $subtitulo = "Los mejores postres en la Casa del Chantilly, calidad y amor";
            $this->tpl->assign("id_pagina", $this->id_pagina);
            $this->tpl->assign("subtitulo", $subtitulo);
            $this->tpl->display("postres.tpl");
        } catch (Exception $e) {
            Session::close();
    ...
    public function refresh_datos_postres()
    {
        ...
        try {
            ...
            $tblpostres = $daoProductoweb->select_all_Productos_postres(["cia" => $this->cia, "pagina"=> $id_pagina,"criterio"=>$criterio], $offset, $limite);
            $total_items = $daoProductoweb->select_count_productos_postres(["cia" => $this->cia, "pagina"=> $id_pagina,"criterio"=>$criterio]);
            $data = [];
            $id = [];
            if (is_array($tblpostres) && count($tblpostres) > 0) {
                foreach ($tblpostres as $fila) {
                    array_push($id, $fila->codart_web);
                }
                $data = list_dismount($tblpostres);
                echo json_encode([
                    "success" => true,
                    "data" => $data,
                    "id" => $id,

...
let metodos = {
    RefreshDatosPromociones: async (pagina = 1, limite = 8) => {
        let criterio = $("#searchInput").val();

        let url = new URL(urlpath + "/Promociones/refresh_datos_promociones/");
        ...
        try {
            ...
            if (resultado.success) {
                Init_cardPromociones(resultado.data,resultado.total_items, resultado.pagina, resultado.limite);
...
let Init_cardPromociones = (datos, totalItems, currentPage, limit) => {
...
let setupPagination = (totalItems, currentPage, limit) => {
    ...
    $('.pagination .page-link').click(function (e) {
        ...
        if (!isNaN(page) && page > 0 && page <= totalPages) {
            metodos.RefreshDatosPromociones(page, limit);
        }
    });
};
...
$(document).ready(() => {
     ...
     metodos.RefreshDatosPromociones();
